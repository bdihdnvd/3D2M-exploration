...
class GridMap
{
 public:
  ...
  inline double getDistWithGradTrilinear(Eigen::Vector3d pos,
                                         Eigen::Vector3d& grad)
  {
    ...
    Eigen::Vector3d pos_m =
        pos - 0.5 * grid_resolution * Eigen::Vector3d::Ones();
...
class PCSmapManager
{
 public:
  ...
  // other functions
  pcl::PointCloud<pcl::PointXYZ> snowProj(
      const sensor_msgs::PointCloud2& globalmap);
