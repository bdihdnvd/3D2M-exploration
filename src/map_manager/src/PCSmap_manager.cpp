...
// STEP1 生成占用栅格地图
  ...
  // STEP1.5 构建ZCOSTMAP
  // 对于空闲的栅格，它会计算该点下方的地面高度，并结合机器人设定的最小/最大通行高度（min_height, max_height），计算出一个“推荐通行高度”
    ...
    for (int j = 0; j < occupancy_map->Y_size; j++)
    {
      for (int k = 0; k < occupancy_map->Z_size; k++)
      {
        ...
        else
        {
          ...
          {
            if (gap > max_height + hex)
            {
              zcost_map->grid_esdf[i][j][k] = max_height;
            }
            else if (gap < min_height + hex)
            {
              zcost_map->grid_esdf[i][j][k] = min_height;
            }
            else
            {
              zcost_map->grid_esdf[i][j][k] = gap - hex;
            }
            zcost_map->grid_esdf[i][j][k] += hg;
          }
        }
        // if(zcost_map -> grid_esdf[i][j][k] < 0){
        //     std::cout<<"1,"<<zcost_map -> grid_esdf[i][j][k]<<" "<<hg<<"
        //     "<<gap<<std::endl;
        // }
      }
    }
  ...
  std::cout << "[MapHandler] STEP1 genrate occupancy map finished!"
            << std::endl;
...
// STEP2 雪地工程
  ...
  for (int i = 0; i < global_cloud.points.size(); i++)
  {
    index = snowed_occupancy_map->getGridIndex(
        Vector3d(global_cloud.points[i].x, global_cloud.points[i].y,
  ...
  for (int i = 0; i < snowed_occupancy_map->X_size; i++)
  {
    for (int j = 0; j < snowed_occupancy_map->Y_size; j++)
    {
      for (int k = 0; k < snowed_occupancy_map->Z_size; k++)
      {
        ...
        {
          snowed_occupancy_map->grid_map[i][j][k] = 0;
        }
      }
    }
  }
  ...
  for (int i = 0; i < snowed_occupancy_map->X_size; i++)
  {
    for (int j = 0; j < snowed_occupancy_map->Y_size; j++)
    {
      for (int k = 1; k < snowed_occupancy_map->Z_size; k++)
      {
        ...
        //                     center_coord = occupancy_map ->
        //                     getGridCubeCenter(i, j, k);
        // #ifdef RENDER_ASTAR_SPACE
        //                         center_coord = occupancy_map ->
        //                         getGridCubeCenter(i, j, k); s_point.x    =
        //                         center_coord(0); s_point.y    =
        //                         center_coord(1); s_point.z    =
        //                         center_coord(2);
        //                         gridmap_vis.push_back(s_point);
        // #endif
        //                 }
        if (snowed_occupancy_map->grid_map[i][j][k - 1] == 1)
        {
          ...
          for (int x_exp = -exp_ind; x_exp <= exp_ind; x_exp++)
          {
            for (int y_exp = -exp_ind; y_exp <= exp_ind; y_exp++)
            {
              ...
              index_exp_y = j + y_exp;
              index_exp_z = k;
              if (snowed_occupancy_map->isIndexValid(index_exp_x, index_exp_y,
                                                     index_exp_z) == false)
              {
                defeat = true;
                break;
              }
              if (!((snowed_occupancy_map->grid_map[index_exp_x][index_exp_y]
                                                   [index_exp_z] == 0 &&
                     index_exp_z - 1 >= 0 &&
                     snowed_occupancy_map->grid_map[index_exp_x][index_exp_y]
                                                   [index_exp_z - 1] != 0) ||
                    (snowed_occupancy_map->grid_map[index_exp_x][index_exp_y]
                                                   [index_exp_z] != 0 &&
  ...
  for (int k = 0; k < N; k++)
  {
    ...
    for (int j = 0; j < occupancy_map->Y_size; j++)
    {
      for (int i = 0; i < occupancy_map->X_size; i++)
      {
        ...
        travelcost_map
            ->travel_grid[dropped_index(0)][dropped_index(1)][dropped_index(2)]
            ->mass_center = surfel_center;
        travelcost_map
            ->travel_grid[dropped_index(0)][dropped_index(1)][dropped_index(2)]
            ->plane_normal = surfel_pose;
        travelcost_map
            ->travel_grid[dropped_index(0)][dropped_index(1)][dropped_index(2)]
            ->explored = true;
      }
  ...
  for (int k = 0; k < N; k++)
  {
    ...
    for (int j = 0; j < travelcost_map->Y_size; j++)
    {
      for (int i = 0; i < travelcost_map->X_size; i++)
      {
        ...
        travelcost_map->travel_grid[index(0)][index(1)][index(2)]->explored =
            true;
        s_point.x = center_coord(0);
        s_point.y = center_coord(1);
        s_point.z = center_coord(2);

        if (global_kdtree.radiusSearch(s_point, radius, pointIdxRadiusSearch,
                                       pointRadiusSquaredDistance) > 0)
        {
          ...
          {
            temp_cloud->points[i] =
                global_pointcloud->points[pointIdxRadiusSearch[i]];
            mass_center(0) =
                (mass_center(0) * i +
                 global_pointcloud->points[pointIdxRadiusSearch[i]].x) /
                (i + 1);
            mass_center(1) =
                (mass_center(1) * i +
                 global_pointcloud->points[pointIdxRadiusSearch[i]].y) /
                (i + 1);
            mass_center(2) =
                (mass_center(2) * i +
                 global_pointcloud->points[pointIdxRadiusSearch[i]].z) /
                (i + 1);
          }
          travelcost_map->travel_grid[index(0)][index(1)][index(2)]
          ...
          travelcost_map->travel_grid[index(0)][index(1)][index(2)]->roughness =
              roughness;
        }
      }
    }
  ...
  {
    cout << "space dS: " << (double)k * 100 / travelcost_map->Z_size << " %"
         << endl;
    for (int j = 1; j < travelcost_map->Y_size - 1; j++)
    {
      for (int i = 1; i < travelcost_map->X_size - 1; i++)
      {
        ...
        if (occupancy_map->isIndexOccupied(c_ind) ||
            occupancy_map->isIndexOccupied(c_indup))
        {
        ...
        dposition =
            (travelcost_map
                 ->travel_grid[cplus_ind(0)][cplus_ind(1)][cplus_ind(2)]
                 ->mass_center -
             2 * travelcost_map->travel_grid[c_ind(0)][c_ind(1)][c_ind(2)]
                     ->mass_center +
             travelcost_map->travel_grid[cmin_ind(0)][cmin_ind(1)][cmin_ind(2)]
                 ->mass_center);
      ...
      for (int j = 0; j < travelcost_map->Y_size - 1; j++)
      {
        for (int k = 0; k < travelcost_map->Z_size; k++)
        {
          ...
          if (cost > 0.2)
          {
            ...
            if (ipoint.intensity > 1000)
            {
              ipoint.intensity = 1000;
            }
