...
{
  surfelmap_vis_pub =
      nh.advertise<visualization_msgs::Marker>("surfel_map_vis", 10000);
}
...
void SurFelMap::renderSurfelMap()
{
  ...
  for (int i = 0; i < surfels.size(); i++)
  {
    ...
    // {
    //     point_pos = surfels[i] -> position + r*cos(t)*surfels[i] -> a +
    //     r*sin(t)*surfels[i] -> b; pt.x = point_pos(0); pt.y = point_pos(1);
    //     pt.z = point_pos(2);
