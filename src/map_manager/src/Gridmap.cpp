...
Vector3i GridMap::getGridIndex(Vector3d pos_w)
{
  if (!isInMap(pos_w))
  {
    if (debug_output)
    {
      ...
      cout << (pos_w(0) <= boundary_xyzmin(0)) << "\t"
           << (pos_w(1) <= boundary_xyzmin(1)) << "\t"
           << (pos_w(2) <= boundary_xyzmin(2)) << "\t"
           << (pos_w(0) >= boundary_xyzmax(0)) << "\t"
           << (pos_w(1) >= boundary_xyzmax(1)) << "\t"
           << (pos_w(2) >= boundary_xyzmax(2)) << "\n";
...
bool GridMap::isOverFloorInCells(Vector3i index, double distance)
{
  ...
  {
    ind(2) = index(2) - dz;
    if (isIndexOccupied(ind)) { return true; }
  }
...
double GridMap::getHeightGap(Vector3d pos_w)
{
  ...
  {
    index_scan(2) = sz;
    // std::cout<<" bz-sz = "<<bz <<" - "<<sz<<"  "<<grid_resolution * (sz -
    // bz)<<std::endl;
    if (isIndexOccupied(index_scan))
    {
...
void GridMap::clearGridPureESDF(double num)
{
  for (int i = 0; i < X_size; i++)
  {
    for (int j = 0; j < Y_size; j++)
    {
      for (int k = 0; k < Z_size; k++) { grid_esdf[i][j][k] = num; }
    }
  }
}
...
void debugP(double num) { cout << num << endl; }
...
void GridMap::generateEmptyESDF()
{
  ...
  for (int x = 0; x < X_size; x++)
  {
    ...
    for (int y = 0; y < Y_size; y++) { grid_esdf[x][y] = new double[Z_size]; }
  }
...
void GridMap::generateESDF1dAt(int ix, int iy, int iz)
{
  ...
  for (int i = iz; i >= 0; i--)
  {
    ...
    num -= grid_resolution;
    if (isIndexOccupied(ix, iy, i) == true) { break; }
  }
  num = -1;
...
void GridMap::generateESDF3d()
{
  ...
  for (int x = min_esdf(0); x <= max_esdf(0); x++)
  {
    for (int y = min_esdf(1); y <= max_esdf(1); y++)
    {
      fillESDF(
          ...
          [&](int z, double val) { grid_esdf_buffer1[x][y][z] = val; },
  ...
  for (int x = min_esdf(0); x <= max_esdf(0); x++)
  {
    for (int z = min_esdf(2); z <= max_esdf(2); z++)
    {
      fillESDF([&](int y) { return grid_esdf_buffer1[x][y][z]; },
               [&](int y, double val) { grid_esdf_buffer2[x][y][z] = val; },
  ...
  for (int x = min_esdf(0); x <= max_esdf(0); ++x)
    for (int y = min_esdf(1); y <= max_esdf(1); ++y)
      for (int z = min_esdf(2); z <= max_esdf(2); ++z)
      {
        if (grid_map[x][y][z] == 0) { grid_map_buffer_neg[x][y][z] = 1; }
        else if (grid_map[x][y][z] == 1) { grid_map_buffer_neg[x][y][z] = 0; }
        else { ROS_ERROR("what?"); }
      }
  ...
  for (int y = min_esdf(1); y <= max_esdf(1); y++)
  {
    ...
    {
      fillESDF(
          [&](int x) { return grid_esdf_buffer2[x][y][z]; },
          [&](int x, double val)
    ...
    for (int y = min_esdf(1); y <= max_esdf(1); ++y)
      for (int z = min_esdf(2); z <= max_esdf(2); ++z)
      {
        ...
        if (grid_map_buffer_neg[x][y][z] > 0.0)
          grid_map_buffer_all[x][y][z] +=
              (-grid_map_buffer_neg[x][y][z] + grid_resolution);
      }
}
...
double GridMap::getSDFValue(Vector3d pos_w)
{
  ...
  double d1 =
      x_r * getGridSDFValue(ix, iy, iz) + x_l * getGridSDFValue(ix + 1, iy, iz);
  double d2 = x_r * getGridSDFValue(ix, iy + 1, iz) +
              x_l * getGridSDFValue(ix + 1, iy + 1, iz);

  double d3 = x_r * getGridSDFValue(ix, iy, iz + 1) +
              x_l * getGridSDFValue(ix + 1, iy, iz + 1);
  double d4 = x_r * getGridSDFValue(ix, iy + 1, iz + 1) +
              x_l * getGridSDFValue(ix + 1, iy + 1, iz + 1);
...
Vector3d GridMap::getSDFGrad(Vector3d pos_w)
{
  if (!isInMap(pos_w))
  {
    if (debug_output) { ROS_WARN("[getSDFGrad()] this point is out of map!"); }
  ...
  double g1 =
      y_r * (getGridSDFValue(ix + 1, iy, iz) - getGridSDFValue(ix, iy, iz)) /
          grid_resolution +
      y_l *
          (getGridSDFValue(ix + 1, iy + 1, iz) -
           getGridSDFValue(ix, iy + 1, iz)) /
          grid_resolution;
  double g2 = y_r *
                  (getGridSDFValue(ix + 1, iy, iz + 1) -
                   getGridSDFValue(ix, iy, iz + 1)) /
                  grid_resolution +
              y_l *
                  (getGridSDFValue(ix + 1, iy + 1, iz + 1) -
                   getGridSDFValue(ix, iy + 1, iz + 1)) /
                  grid_resolution;
  double gx = z_r * g1 + z_l * g2;

  double g3 =
      x_r * (getGridSDFValue(ix, iy + 1, iz) - getGridSDFValue(ix, iy, iz)) /
          grid_resolution +
      x_l *
          (getGridSDFValue(ix + 1, iy + 1, iz) -
           getGridSDFValue(ix + 1, iy, iz)) /
          grid_resolution;
  double g4 = x_r *
                  (getGridSDFValue(ix, iy + 1, iz + 1) -
                   getGridSDFValue(ix, iy, iz + 1)) /
  ...
  double g5 =
      x_r * (getGridSDFValue(ix, iy, iz + 1) - getGridSDFValue(ix, iy, iz)) /
          grid_resolution +
      x_l *
          (getGridSDFValue(ix + 1, iy, iz + 1) -
           getGridSDFValue(ix + 1, iy, iz)) /
...
void TravelGridMap::releaseMemory()
{
  ...
  for (int x = 0; x < X_size; x++)
  {
    for (int y = 0; y < Y_size; y++) { delete[] travel_grid[x][y]; }
  }
  for (int x = 0; x < X_size; x++) { delete[] travel_grid[x]; }
  delete[] travel_grid;
}

void TravelGridMap::createGridMap(Vector3d boundary_xyzmin,
...
{
  travel_grid[tgt_index(0)][tgt_index(1)][tgt_index(2)]->explored =
      travel_grid[src_index(0)][src_index(1)][src_index(2)]->explored;
  travel_grid[tgt_index(0)][tgt_index(1)][tgt_index(2)]->plane_normal =
      travel_grid[src_index(0)][src_index(1)][src_index(2)]->plane_normal;
  travel_grid[tgt_index(0)][tgt_index(1)][tgt_index(2)]->mass_center =
      travel_grid[src_index(0)][src_index(1)][src_index(2)]->mass_center;
  travel_grid[tgt_index(0)][tgt_index(1)][tgt_index(2)]->roughness =
      travel_grid[src_index(0)][src_index(1)][src_index(2)]->roughness;
}
...
bool TravelGridMap::isTravelGridExplored(Vector3i index)
{
  ...
  if (travel_grid[index(0)][index(1)][index(2)]->explored == 0)
  {
    return false;
  }
  else { return true; }
...
Vector3d TravelGridMap::getTravelPN(Vector3d pos_w)
{
  if (!isInMap(pos_w))
  {
    if (debug_output) { ROS_WARN("[getTravelPN()] this point is out of map!"); }
    return Vector3d(0, 0, 0);
  }
...
void TravelGridMap::clearGridESDF()
{
  for (int i = 0; i < X_size; i++)
  {
    for (int j = 0; j < Y_size; j++)
    {
      for (int k = 0; k < Z_size; k++) { grid_esdf[i][j][k] = 0; }
    }
  }
}
  ...
  for (int z = 0; z < Z_size; z++)
  {
    ...
    for (int y = 1; y < Y_size - 1; y++)
    {
      for (int x = 1; x < X_size - 1; x++)
      {
        if (grid_map[x][y][z] > 2)
        {
          occ_ids.push_back(Vector2i(x, y));
          grid_esdf[x][y][z] = grid_map[x][y][z];
        }
      }
    }
      ...
      for (int dx = -turcation_id; dx <= turcation_id; dx++)
      {
        for (int dy = -turcation_id; dy <= turcation_id; dy++)
        {
          ...
          if (!(dx == 0 && dy == 0) && isIndexValid(ix, iy, z))
          {
            ...
            grid_esdf[ix][iy][z] =
                max(grid_esdf[ix][iy][z],
                    grid_esdf[sx][sy][z] * (turcation - distance) / turcation);
          }
        }
      }
    }
